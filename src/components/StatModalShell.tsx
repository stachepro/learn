  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      <div
        ...
        style={{ background: 'rgb(var(--scrim))', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)' }}
        onClick={close}
      />

      <SurfaceCard
        variant="raised"
        className={`stat-modal-card relative w-full max-w-md flex flex-col ${isExiting ? 'sheet-exit' : 'sheet-enter'}`}
        style={{ maxHeight: 'calc(100dvh - env(safe-area-inset-top) - 40px)' }}
      >
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid rgb(var(--border-subtle))' }}>
          <div className="flex items-center gap-2.5 min-w-0">
            {headerIcon}
            <div className="min-w-0">
              <h2 className="type-section-title text-lg truncate" style={{ color: 'rgb(var(--text-primary))' }}>{title}</h2>
              {subtitle && <p className="type-caption mt-0.5 truncate" style={{ color: 'rgb(var(--text-tertiary))' }}>{subtitle}</p>}
            </div>
          </div>
          <button
            onClick={close}
